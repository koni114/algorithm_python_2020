...
tomato = [list(map(int, f.readline().strip().split(" "))) for _ in range(N)]
...
day = -1
def isTomatoAllGood():
    isTomatoAllGood = True
    for i in range(N):
        for j in range(M):
            if tomato[i][j] == 0:
                isTomatoAllGood = False
            elif tomato[i][j] == 1:
                check[i][j] = 1
                d.appendleft([i, j, 0])
            else:
    ...
    while(len(d) != 0):
        ...
        for i in range(4):
            ...
            if nx >= 0 and nx < N and ny >= 0 and ny < M:
                if tomato[nx][ny] == 0 and check[nx][ny] == 0:
                    check[nx][ny] = c+1
