...
for _ in range(T):
    ...
    tmp1 = list(map(int, f.readline().strip().split(" ")))
    tmp2 = list(map(int, f.readline().strip().split(" ")))
    sticker = []
    sticker.append(tmp1)
    sticker.append(tmp2)
    if N <= 1:
        if N == 1: print(max(sticker[0][0], sticker[1][0]))
        if N == 2: print(max(sticker[0][1] + sticker[1][0], sticker[1][0] + sticker[0][1]))
    else:
