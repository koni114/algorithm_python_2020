...
# 문제
# 효주는 포도주 시식회에 갔다.
# 그 곳에 갔더니, 테이블 위에 다양한 포도주가 들어있는 포도주 잔이 일렬로 놓여 있었다. 효주는
...
wine.append(0)
for _ in range(N):
    wine.append(int(f.readline().strip()))

if N <= 3:
    if N == 1: print(wine[1])
    if N == 2: print(wine[1] + wine[2])
    if N == 3: print(max(wine[1] + wine[2], wine[3] + max(wine[2], wine[1])))
else:
    dp = [0 for _ in  range(N+1)]
    dp[1] = wine[1]
    dp[2] = wine[1] + wine[2]
    dp[3] = wine[3] + max(wine[2], wine[1])
    for i in range(4, N+1):
        dp[i] = wine[i] + max(dp[i-2], wine[i-1] + max(dp[i-3], dp[i-4]))
